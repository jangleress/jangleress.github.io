...
exports.init = function() {
  ...
  /* tf: added infinity check for browsers that can't handle large numbers (eg chrome 37 on iOSX 10.6.8*/
  var mtan1 = Math.tan(0.5 * this.phic0 + FORTPI)
  var mtan2 = Math.tan(0.5 * this.lat0 + FORTPI)
  mtan1 = mtan1 !== Number.POSITIVE_INFINITY ? mtan1 : Number.MAX_VALUE
  mtan2 = mtan2 !== Number.POSITIVE_INFINITY ? mtan2 : Number.MAX_VALUE
  this.K = mtan1 / (Math.pow(mtan2, this.C) * srat(this.e * sphi, this.ratexp));
};
...
exports.forward = function(p) {
  ...
  var lat = p.y;
  /* tf: added infinity check for browsers that can't handle large numbers (eg chrome 37 on iOSX 10.6.8*/
  var mtan = Math.tan(0.5 * lat + FORTPI)
  mtan = mtan !== Number.POSITIVE_INFINITY ? mtan : Number.MAX_VALUE
  p.y = 2 * Math.atan(this.K * Math.pow(mtan, this.C) * srat(this.e * Math.sin(lat), this.ratexp)) - HALF_PI;
  p.x = this.C * lon;
  return p;
};
...
exports.inverse = function(p) {
  ...
  var mtan = Math.tan(0.5 * lat + FORTPI)
  mtan = mtan !== Number.POSITIVE_INFINITY ? mtan : Number.MAX_VALUE  
  var num = Math.pow(mtan / this.K, 1 / this.C);
